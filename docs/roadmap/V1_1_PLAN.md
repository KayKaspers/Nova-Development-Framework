...
| G-13 (Weg A) | External Validation Evidence Depth | future improvement | **WP-153** | **P1** | gering (additive Evidenz) | größter v1.0-Vorbehalt; für v1.1 stärken |
| — | „kein externes RC-Feedback" | accepted limitation → **teilweise adressiert** | **WP-153/WP-154** | P1 | gering | erster Projekt-Feedback-Intake (project-local) speist Enablement/Feedback-Triage |
| — | Project Enablement / weitere Skills | candidate / future improvement | **WP-154** | P1 | gering | Adapter/Enablement praktisch validieren |
| — | Public Documentation Polish | planned | **WP-155** | P2 | gering | Doku-Feinschliff, keine Substanzänderung |
| — | zweiter Fixture-Typ (Projekt mit Code) | non-goal (v1.0) → candidate (v1.1) | **WP-153** (optional) | P3 | gering | optional in External Validation; sonst weiter Non-Goal |
...
| — | v1.x-Kompatibilität aktiv | planned (maintenance) | **WP-156/157** | P1 | — | Breaking-/Deprecation-Regeln pflegen (ADR-0031) |

## EN – Known Final Notes → Roadmap Mapping

The table maps each note to a post-v1.0 status, a recommended WP, a priority (P0–P3), a v1.x-compatibility risk, and a rationale: RDS-001/ADS-001 and the 38-skill real-use history → WP-151 (skills real-use and context efficiency review, done); token/context efficiency (from WP-151) → WP-152 (token efficiency & context budget baseline); G-13 path A → WP-153 (external validation improvement); the "no external RC feedback" limitation is now partially addressed by the first project feedback intake → WP-153/154; project enablement/further skills → WP-154; public docs polish → WP-155; a second fixture type moves from non-goal to a v1.1 candidate (optional in WP-153); the single-maintainer bottleneck stays an accepted structural limitation (tracked); v1.x compatibility maintenance → WP-156/157.
...
| **WP-152 – Token Efficiency & Context Budget Baseline** *(neu, aus WP-151)* | additive, docs-only Token-Efficiency-Baseline umsetzen | Context Budgets B0–B4; Lean/Handoff/Review-only/Fix-Profile (additiv); Templates; Short Report; Evidence Pack; Skill-Selektion; Fail-Closed; project-local Pilot | neue Skills, Skill-Umbau, Subsystem, externe Tools/MCP/Netz, v1.1 Scope Lock/Release | Baseline-Guide + Templates + Validation-Doc + Notes ([Guide](../guides/TOKEN_EFFICIENCY_AND_CONTEXT_BUDGET_BASELINE.md)) | Skill-assisted Standard/Extended | Fable 5 / Opus 4.8 | gering (additiv) | GO WITH NOTES |
| **WP-153 – External Validation Improvement** | G-13-Weg A angehen | tieferer öffentlicher schrittbelegter Neutral-Lauf; ggf. zweiter Fixture-Typ; Feedback-Intake einordnen | erfundene Evidenz, private Daten | Validation-Doc + Notes | Skill-assisted Full | Opus 4.8 | gering (additiv) | GO WITH NOTES |
| **WP-154 – Project Enablement Validation** | Adapter/Enablement praktisch validieren | Adapter-Qualität/Neutralität; ersten Projekt-Feedback-Intake neutral einbeziehen | private Projektlogik im Public NDF, Auto-Migration | Validation-Doc + Notes | Skill-assisted Full | Opus 4.8 | gering | GO WITH NOTES |
| **WP-155 – Public Documentation Polish** | Doku-Feinschliff | README/Guides/i18n-Matrix; Klarheit/Konsistenz | Substanz-/Governance-Änderung | Doku-Updates + Notes | Skill-assisted Standard | Opus 4.8 | gering | GO WITH NOTES |
| **WP-156 – v1.1 Readiness Review** | v1.1-Reife prüfen | Check gegen v1.1-Scope + v1.x-Kompatibilität | Release-Erstellung | Readiness-Doc + Notes | Skill-assisted Full | Opus 4.8 | mittel | GO WITH NOTES / REWORK |
| **WP-157 – v1.1 Release Prep** | v1.1-Release vorbereiten (nur Doku) | Release Notes/Go-No-Go/Tagging-Guide (Tag `v1.1.0`) | Tag/Release durch AI | Release-Prep-Docs + Notes | Skill-assisted Full | Opus 4.8 | mittel | GO WITH NOTES – pending Human Maintainer |

**Begründung der Reihenfolge:** zuerst Betriebs-Evidenz der Skills (151, erledigt), dann die daraus abgeleitete additive Token-Efficiency-Baseline (152, geringstes Risiko, verbessert alle Folge-WPs), dann der größte v1.0-Vorbehalt External Validation (153), dann Enablement mit realem Feedback (154), dann Doku-Polish (155), dann Readiness (156) und Release Prep (157). Optional/parallel und nicht blockierend: WP-130/131/132 (0.9-Optional-Assessments).

## EN – Work Packages (post-v1.0 order)

Reconciliation note: after WP-151 completed, its recommended additive token-efficiency work is inserted as a new **WP-152 – Token Efficiency & Context Budget Baseline**, and the former WP-152…156 shift to **WP-153…157**. The table sequences WP-151 (skills real-use and context efficiency review, done) → WP-152 (token efficiency & context budget baseline) → WP-153 (external validation improvement, G-13 path A) → WP-154 (project enablement validation) → WP-155 (public documentation polish) → WP-156 (v1.1 readiness review) → WP-157 (v1.1 release prep, docs only). Rationale: operational skill evidence first (done), then the additive efficiency baseline derived from it (lowest risk, improves every following WP), then external validation, enablement, docs polish, readiness, release prep. No further roadmap expansion.
...
**GO WITH NOTES – v1.1 planning started.** v1.0.0 bleibt final veröffentlicht; v1.x-Zusage aktiv; v1.1 = Validation, Enablement & Operational Maturity, nur geplant (nicht released); Known Final Notes in WP-151…157 überführt (WP-152 Token-Efficiency-Einschub); keine Breaking Changes, keine neuen Skills, keine Releases. ADR-0031/0032 unverändert. WP-151 ist abgeschlossen (Skills Real-use and Context Efficiency Review). **Nächster empfohlener WP: WP-152 – Token Efficiency & Context Budget Baseline** *(additiver, docs-only Einschub aus der WP-151-Empfehlung; bisherige WP-152…156 rücken zu WP-153…157).*

## EN – Decision

**GO WITH NOTES – v1.1 planning started.** v1.0.0 stays final; the v1.x promise is active; v1.1 = Validation, Enablement & Operational Maturity, planned only (not released); known final notes mapped into WP-151…157 (WP-152 token-efficiency insert); no breaking changes, new skills, or releases. ADR-0031/0032 unchanged. WP-151 is completed (skills real-use and context efficiency review); next recommended WP: WP-152 – Token Efficiency & Context Budget Baseline.
