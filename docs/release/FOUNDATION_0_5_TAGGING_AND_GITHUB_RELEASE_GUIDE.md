...
- Go/No-Go-Checkliste vollständig mit **GO** abgeschlossen (`FOUNDATION_0_5_GO_NO_GO_CHECKLIST.md`).
- WP-082-Stand ist committet und gepusht.
- GitHub Secret `NDF_PUBLIC_NEUTRALITY_DENYLIST` gesetzt (seit WP-070 verifiziert; macht den new-file neutrality check in CI scharf).
...
Erwartung: Der WP-082-Commit ist der aktuelle Stand von `origin/main`.

## 4. Prüfen, ob der Tag bereits existiert / Check whether the tag already exists

```powershell
git tag --list v0.5.0-foundation
git ls-remote --tags origin v0.5.0-foundation
```
...
```powershell
git tag -a v0.5.0-foundation -m "Nova Development Framework v0.5.0 Foundation"
```
...
```text
Nova Development Framework v0.5.0 Foundation
```
...
Aus `docs/release/FOUNDATION_0_5_RELEASE_NOTES.md` ableiten — der fertige Vorschlag steht dort im Abschnitt „GitHub Release Body (Vorschlag / suggested)". Keine privaten Begriffe einfügen; „This is not a v1.0 release" beibehalten.

## 10. Nachprüfung / Verification

- [ ] Release erscheint auf GitHub als **Pre-release** mit Tag `v0.5.0-foundation` auf `main`.
...
- [ ] `git tag` zeigt `v0.1.0-foundation` bis `v0.5.0-foundation`.
...
```powershell
git tag -d v0.5.0-foundation
git push origin :refs/tags/v0.5.0-foundation
```

- **Bereits öffentlich genutzt:** Tag nicht mehr verschieben — stattdessen Korrektur-Release (z. B. `v0.5.1-foundation`). Kein History-Rewrite.

## 12. Post-Release-Cleanup / Post-release cleanup

Nach Tag + Release: Post-Release-Status-Cleanup als eigenes WP (Muster WP-043/056/069) — Changelog/README/Release-Notes von „tag pending/vorbereitet" auf „released" heben, Tag und Release read-only verifizieren, Go/No-Go und dieser Guide als historisch markieren.
