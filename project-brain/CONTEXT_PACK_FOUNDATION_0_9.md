...
`NDF-WP-154 – Skills Pack P0 Hardening` — Nova-Review bestanden; die Human-Maintainer-Annahme wird mit dem Commit wirksam, der diesen Stand trägt ([Validation](../docs/validation/v1-1/SKILLS_PACK_P0_HARDENING.md)). Genau sechs Skills gehärtet — `ndf-work-package-runner` als primärer docs-only Ausführungs-Router (Execution Header, Profile/Budgets ohne Absenkung, Fail-Closed-Eskalation, 0–3 Support-Skills, Prompt-vor-Skill-Ausgabevorrang, keine Runtime), `ndf-compact-context-summary-runner` (Formatvorrang statt fester Zwei-Block-Form, Handoff-Essentials), `ndf-changelog-writer` / `ndf-release-safety` / `ndf-release-notes-runner` versionsneutral (v1.x-Zusage aktiv seit `v1.0.0`), `ndf-v1-readiness-review` historisch/Spezialist → `ndf-release-safety`; Skills-README reconciled. Keine neuen/entfernten/umbenannten Skills, keine Provider-Mechanik, keine ADR-Änderung; Integrity-Records der sechs Skills veraltet → Consumer re-verifizieren. Davor: WP-153 (committed `70446a3`) und WP-152 (committed `c4c1c34`).

## Next Work Package

**NDF-WP-155 – External Validation Improvement** *(nächster geplanter WP — nicht gestartet)*: G-13-Weg A angehen (tieferer öffentlicher schrittbelegter Neutral-Lauf; ggf. zweiter Fixture-Typ; Projekt-Feedback-Intake neutral einordnen). **Keine autorisierte Ausführung, solange kein eigener vollständiger WP-155-Ausführungsprompt mit Execution Header (`SESSION`/`STATUS`) vorliegt.** ADR-0032-Consumer-Scope → separate, eigens autorisierte ADR (Kandidat ADR-0033). WP-130/131/132 bleiben optional/nicht aktiviert.
...
- WP-154: Skills Pack P0 Hardening — Nova-Review bestanden; Annahme wirksam mit dem Commit, der diesen Stand trägt; sechs SKILL.md gehärtet + Skills-README; Klarstellung/non-breaking/additiv, kein Breaking Change; Integrity-Lock-Refresh für die sechs Skills nötig; keine ADR-Änderung; v1.1 planning only.

## Next Prompt Recommendation

**NDF-WP-155 – External Validation Improvement** als eigener, vollständiger Ausführungsprompt mit Execution Header (`SESSION`/`STATUS`) — bis dahin keine autorisierte Ausführung. Consumer mit Integrity Locks re-verifizieren die sechs geänderten Skills vor normativer Nutzung. ADR-0032-Consumer-Scope nur per separater, eigens autorisierter ADR. WP-130/131/132 bleiben optional/nicht aktiviert.
