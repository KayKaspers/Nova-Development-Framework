...
`NDF-WP-155 – External Validation Improvement` — die Human-Maintainer-Annahme wird mit dem Commit wirksam, der diesen Stand trägt ([Validation](../docs/validation/v1-1/EXTERNAL_VALIDATION_IMPROVEMENT.md)). G-13-Weg A ausgeführt: das öffentliche [Runbook](../docs/validation/project-adapter/INDEPENDENT_ADAPTER_VALIDATION_RUNBOOK.md) vollständig durchlaufen — **alle sechs Schritte und alle elf Adapter-Phasen einzeln belegt** (genau der Nachweis, den WP-088 als „not provided" hinterließ) — gegen ein generisches, NDF-fremdes, **code-tragendes** Fixture außerhalb des Repos (zwei Pakete, Python + JavaScript, drei widersprüchliche Versionsquellen, drei ungesicherte irreversible Operationen, kein CI, fehlschlagende Testsuite). Ergebnis **PASS WITH NOTES**, kein Blocker; 15 deterministische Checks getrennt vom semantischen Review; acht Findings (EVI-001…008) — nur erfasst, **nicht behoben** (alle betroffenen Dateien außerhalb des autorisierten Scopes). **G-13 → `MATERIALLY_REDUCED`, nicht geschlossen:** Tiefen-Limb adressiert (PSV-001 und beide E-03-Grenzen), Unabhängigkeits-Limb unverändert — der Lauf war agenten-ausgeführt und agenten-autoriert. Keine project-local, Human-Maintainer- oder unabhängige Evidenz genutzt oder erfunden. Davor: `NDF-WP-154 – Skills Pack P0 Hardening` (committed `07b2884`, [Validation](../docs/validation/v1-1/SKILLS_PACK_P0_HARDENING.md)) — genau sechs Skills gehärtet — `ndf-work-package-runner` als primärer docs-only Ausführungs-Router (Execution Header, Profile/Budgets ohne Absenkung, Fail-Closed-Eskalation, 0–3 Support-Skills, Prompt-vor-Skill-Ausgabevorrang, keine Runtime), `ndf-compact-context-summary-runner` (Formatvorrang statt fester Zwei-Block-Form, Handoff-Essentials), `ndf-changelog-writer` / `ndf-release-safety` / `ndf-release-notes-runner` versionsneutral (v1.x-Zusage aktiv seit `v1.0.0`), `ndf-v1-readiness-review` historisch/Spezialist → `ndf-release-safety`; Skills-README reconciled. Keine neuen/entfernten/umbenannten Skills, keine Provider-Mechanik, keine ADR-Änderung; Integrity-Records der sechs Skills veraltet → Consumer re-verifizieren. Davor: WP-153 (committed `70446a3`) und WP-152 (committed `c4c1c34`).

## Next Work Package

**NDF-WP-156 – Project Enablement Validation** *(nächster geplanter WP — nicht gestartet)*: Adapter/Enablement praktisch validieren; ersten Projekt-Feedback-Intake neutral einbeziehen. WP-155-Finding **EVI-007** (Phase 0 setzt einen mitwirkenden Human Maintainer voraus) ist relevanter Input; **EVI-001…006 und EVI-008** gehören zu WP-157 (Documentation Polish) und bleiben in WP-156 unbehoben. **Keine autorisierte Ausführung, solange kein eigener vollständiger WP-156-Ausführungsprompt mit Execution Header (`SESSION`/`STATUS`) vorliegt.** ADR-0032-Consumer-Scope → separate, eigens autorisierte ADR (Kandidat ADR-0033). WP-130/131/132 bleiben optional/nicht aktiviert.
...
**NDF-WP-156 – Project Enablement Validation** als eigener, vollständiger Ausführungsprompt mit Execution Header (`SESSION`/`STATUS`) — bis dahin keine autorisierte Ausführung. WP-156 darf nicht vor Nova-Review und Human-Maintainer-Annahme von WP-155 starten. Consumer mit Integrity Locks re-verifizieren die sechs in WP-154 geänderten Skills vor normativer Nutzung. ADR-0032-Consumer-Scope nur per separater, eigens autorisierter ADR. WP-130/131/132 bleiben optional/nicht aktiviert.
