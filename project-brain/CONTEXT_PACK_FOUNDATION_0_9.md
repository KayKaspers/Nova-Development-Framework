...
`NDF-WP-137 – Docs-only Extended Core Skills MVP Implementation` — GO WITH NOTES; genau vier docs-only Extended-Core-Skills unter `.claude/skills/` implementiert (`ndf-skill-quality-reviewer`, `ndf-existing-project-analysis-runner`, `ndf-docs-polish-runner`, `ndf-changelog-writer`) + README-Update; keine optionalen +2 (`ndf-release-safety`, `ndf-adr-governance-review`); bestehende MVP-Skills unverändert; Skill-Pack nun acht Skills; ADR-0032 unverändert. Validation: `docs/validation/foundation-0-9/EXTENDED_CORE_SKILLS_MVP_VALIDATION.md`.

## Next Work Package

**NDF-WP-138 – Skill Prompt Compression Real-use Validation** (Kandidat, docs-only; schließt DSK-001/ECS-001 mit realer Vorher/Nachher-Messung; empfohlenes Modell Claude Opus 4.8). Aktualisierte Roadmap: WP-138 → später Project-Enablement-Blueprint → v1.0 Gap Review & Scope Lock. Optionale +2 Core-Skills bleiben Kandidaten für ein späteres, gescoptes WP. WP-130/131/132 bleiben optional/nicht aktiviert.
...
- WP-137: Docs-only Extended Core Skills MVP Implementation — **GO WITH NOTES**; genau vier docs-only Extended-Core-Skills unter `.claude/skills/` implementiert (skill-quality-reviewer, existing-project-analysis-runner, docs-polish-runner, changelog-writer) + README-Update + [Validation](../docs/validation/foundation-0-9/EXTENDED_CORE_SKILLS_MVP_VALIDATION.md) (18-Punkte-Matrix); keine optionalen +2; bestehende MVP-Skills unverändert; Skill-Pack nun **acht** Skills; keine Scripts/Netz/Secrets/privaten Daten/Git-Release-Aktionen; ADR-0032 unverändert. Known Note ECS-001: Token-Ersparnis noch nicht empirisch gemessen (→ WP-138).

## Next Prompt Recommendation

**Skill-assisted Standard/Full Prompt Mode** für **NDF-WP-138 – Skill Prompt Compression Real-use Validation** (reale Vorher/Nachher-Messung an echten WPs; schließt DSK-001/ECS-001; empfohlenes Modell Claude Opus 4.8). Danach später Project-Enablement-Blueprint → v1.0 Gap Review & Scope Lock. WP-130/131/132 bleiben optional/nicht aktiviert.
