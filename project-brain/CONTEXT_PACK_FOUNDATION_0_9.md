...
`NDF-WP-156 – Project Enablement Validation` — die Human-Maintainer-Annahme ist mit Commit `270c6c5` wirksam ([Validation](../docs/validation/v1-1/PROJECT_ENABLEMENT_VALIDATION.md)). Öffentlicher **Enablement-Dry-Run**: Phase-0-(Intake)- und Phase-1-(Read-only-Review)-Mechanik feldweise gegen das bereits öffentliche WP-155-Archetyp geprüft — ohne Adapter-Installation, ohne reales Projekt, ohne Consumer-Skill-Install. Ergebnis: der Pfad ist in seiner **PREPARED**-Hälfte (agentenseitig, read-only ableitbar: Project Type, Tech Stack, Known Risks, teilweise Project Name/Deployment/Repository URL) portabel und in seiner **AUTHORIZED**-Hälfte (Maintainer Goals, Public/Private Status, Safety Notes) korrekt Human-Maintainer-gebunden — erstmals so benannt und feldweise belegt. **EVI-007 (aus WP-155) → `CLARIFIED`:** Phase 0 ist eine bewusste Governance-Grenze, kein Defekt. Fünf neue Findings (PEV-001…005) — Terminologie-Namespacing-Lücke (`NDF-B0`…`NDF-B4`/`NDF-Lean`), EVI-007 im Adapter-Guide selbst unbelegt, fehlender Feldabgleich im `ndf-existing-project-analysis-runner`-Skill, dünnes Enablement-Feedback-Signal, keine PREPARED/AUTHORIZED-Markierung im Intake-Template — nur erfasst, **nicht behoben** (alle Zieldateien außerhalb des autorisierten Scopes). Minimales Enablement-Artefaktset: fünf REQUIRED-Artefakte (Project Brief/Intake, Existing Project Analysis, Adapter-Entscheidung, Scope-Grenze, Next Authorised Action); Rest optional/situativ. Erster Projekt-Feedback-Intake neutral einbezogen (1 von 7 Kandidaten enablement-relevant; dessen eigene Adoption bleibt separat pending). Evidence-only/additiv; keine Skill-/ADR-/Adapter-/Framework-Änderung. Davor: `NDF-WP-155 – External Validation Improvement` (Human-Maintainer-Annahme wirksam mit `243225d`, [Validation](../docs/validation/v1-1/EXTERNAL_VALIDATION_IMPROVEMENT.md)) — G-13-Weg A, PASS WITH NOTES, **G-13 `MATERIALLY_REDUCED`, nicht geschlossen**.

## Next Work Package

**NDF-WP-157 – Public Documentation Polish** *(umgesetzt — Nova-Review bestanden, Human-Maintainer-Annahme wird mit dem Commit wirksam, der diesen Stand trägt; [Validation](../docs/validation/v1-1/PUBLIC_DOCUMENTATION_POLISH.md))*: WP-155-Findings **EVI-001…006 und EVI-008** (Phase-3-Manifestformat, Output-Struktur, DE-only-Dokumente) sowie WP-156-Findings **PEV-001, PEV-002, PEV-003, PEV-005** (Terminologie-Namespacing, EVI-007-Dokumentation, Skill-Feldabgleich, Intake-Template-Markierung) dokumentarisch aufgelöst — Manifest-Spec und drei geschiffte YAML-Manifeste auf das bereits etablierte kanonische Markdown-Format (`PROJECT_MANIFEST.md`, seit WP-059) ausgerichtet statt eine neue Repräsentation zu erfinden; Output-Struktur-Minimal-/Vollvariante an die Phase-Tabelle angeglichen; drei zuvor DE-only Adapter-Pfad-Dokumente (Guide, Manifest-Spec, Intake-Template) bilingual gemacht; PREPARED/AUTHORIZED- und Terminologie-Namespacing (`NDF-B0`…`NDF-B4`/`NDF-Lean`)-Klarstellung im Guide bzw. Baseline-Guide ergänzt; PEV-003 rein dokumentarisch gelöst (kein SKILL.md geändert). PEV-004 (dünnes Enablement-Feedback-Signal) bleibt accepted limitation. Eine historische Public-Neutrality-Frage (eine Dateifamilie mit externem Projektnamen in Dateinamen, WP-IDs und einem CHANGELOG-Eintrag) wurde zunächst als `HISTORICAL_PATH_NEUTRALITY_DECISION_REQUIRED` zurückgemeldet und danach auf explizite Human-Maintainer-Freigabe (Option A) migriert: 8 Dateien auf ein neutrales `CROSS_PROJECT_FEEDBACK_001`/`CPF`-Schema umbenannt, 11 IDs migriert, 10 weitere Dateien inhaltlich neutralisiert — keine Git-Historie umgeschrieben, kein Tag/Release verändert. Keine ADR-/Skill-Verhaltens-/Adapter-Redesign-Änderung. ADR-0032-Consumer-Scope → separate, eigens autorisierte ADR (Kandidat ADR-0033). WP-130/131/132 bleiben optional/nicht aktiviert. **Nächster geplanter WP: WP-158 – v1.1 Readiness Review** — nicht gestartet; startet erst nach Human-Maintainer-Annahme (Commit) von WP-157.
...
**NDF-WP-158 – v1.1 Readiness Review** als eigener, vollständiger Ausführungsprompt mit Execution Header (`SESSION`/`STATUS`) — bis dahin keine autorisierte Ausführung. WP-157 ist umgesetzt (Nova-Review bestanden; Human-Maintainer-Annahme wird mit dem Commit wirksam, der diesen Stand trägt); WP-158 darf nicht vor der Human-Maintainer-Annahme (Commit) von WP-157 starten. Consumer mit Integrity Locks re-verifizieren die sechs in WP-154 geänderten Skills vor normativer Nutzung. ADR-0032-Consumer-Scope nur per separater, eigens autorisierter ADR. WP-130/131/132 bleiben optional/nicht aktiviert.
