...
`NDF-WP-151 – Skills Real-use and Context Efficiency Review` — GO WITH NOTES (committed `ed2195a`); 38 Skills klassifiziert (4 häufig / 9 gelegentlich / 25 selten; Merge-/Adapter-Kandidaten, keine Entfernung/Umbenennung); Token-/Kontextproblem belegt; Kontextklassen + Context Budgets B0–B4 + additive Lean-/Handoff-/Review-only-/Fix-Profile + Lean-WP-Prompt-Architektur + SESSION_HANDOFF-/CURRENT_STATE-Empfehlung; neue Skills `ndf-token-budget-reviewer`/`ndf-session-handoff-runner` abgelehnt (Guidance/Template statt Skill); WP-152-Entscheidungsvorlage: GO additiv-schlank + project-local Pilot. Doc: `docs/validation/v1-1/SKILLS_REAL_USE_AND_CONTEXT_EFFICIENCY_REVIEW.md`.

## Next Work Package

**NDF-WP-152 – Token Efficiency & Context Budget Baseline** *(umgesetzt, pending Nova review — [Guide](../docs/guides/TOKEN_EFFICIENCY_AND_CONTEXT_BUDGET_BASELINE.md))*: additive, docs-only Baseline aus WP-151 — Context Budgets B0–B4 (B1 Lean bevorzugter Normalfall, B4 kein Standard), additive Prompt-Profile Lean/Handoff/Review-only/Fix (Full/Standard/Short unverändert), 8-Element-WP-Kern, fünf Templates (CURRENT_STATE/SESSION_HANDOFF/LEAN_WP/REVIEW_ONLY/FIX), Short Report, Evidence Pack, Skill-Selektionsregel, Fail-Closed-Regeln, neutraler operations-Archetyp-Pilot. Keine neuen/entfernten/umbenannten Skills, keine Scripts/Tools/MCP/Netz, keine Git-/Release-Aktion; ADR-0031/0032 unverändert; v1.1 planning only. Roadmap-Einschub: neue Reihenfolge **WP-152 … WP-157** (bisherige WP-152…156 → WP-153…157). WP-130/131/132 bleiben optional/nicht aktiviert.
...
- WP-152: Token Efficiency & Context Budget Baseline — **GO WITH NOTES – baseline implemented** (pending Nova review, [Guide](../docs/guides/TOKEN_EFFICIENCY_AND_CONTEXT_BUDGET_BASELINE.md)); additiv/docs-only: Baseline-Guide, Budgets B0–B4, Profile Lean/Handoff/Review-only/Fix, fünf Templates, Short Report, Evidence Pack, Skill-Selektion, Fail-Closed, project-local Pilot; keine neuen/entfernten/umbenannten Skills, keine Scripts/Tools/MCP/Netz/Git-Aktion; ADR-0031/0032 unverändert; Roadmap → WP-152…157; v1.1 planning only.

## Next Prompt Recommendation

**Human-Maintainer-Commit von WP-152** (`docs(context): add token efficiency baseline`), danach **project-local Pilot** im Operations-Archetyp; anschließend Bewertung einer NDF-Core-Härtung bzw. WP-153 (External Validation Improvement, G-13-Weg A). WP-130/131/132 bleiben optional/nicht aktiviert.
