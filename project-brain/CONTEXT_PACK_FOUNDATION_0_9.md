...
`NDF-WP-152 – Token Efficiency & Context Budget Baseline` — GO WITH NOTES (committed `c4c1c34`); additive, docs-only Baseline: Context Budgets B0–B4, Profile Lean/Handoff/Review-only/Fix, fünf Templates, Short Report, Evidence Pack, Skill-Selektion, Fail-Closed, project-local Pilot. Doc: `docs/guides/TOKEN_EFFICIENCY_AND_CONTEXT_BUDGET_BASELINE.md`. Danach: reale project-local Pilot-Evidenz und ein read-only **Skills Pack Optimization Review** (abgeschlossen, kein Repository-Artefakt; Befunde → WP-153/WP-154; Consumer-Scope-Frage zu ADR-0032 → separate ADR).

## Next Work Package

**NDF-WP-153 – Prompt Execution Contract Baseline** *(umgesetzt — Annahme erst durch Nova-Review + Human-Maintainer-Commit; [Execution Contract](../framework/prompts/blocks/BLOCK_EXECUTION_CONTRACT.md) · [Validation](../docs/validation/v1-1/PROMPT_EXECUTION_CONTRACT_BASELINE.md))*: framework-weiter, docs-only Ausführungsvertrag — Session Declaration (`SAME_SESSION_ALLOWED` / `SAME_SESSION_RECOMMENDED` / `NEW_SESSION_RECOMMENDED` / `NEW_SESSION_REQUIRED`; Legacy-Fallback `NEW_SESSION_RECOMMENDED` + `SESSION_DECLARATION_MISSING_LEGACY`), Complete Execution Instruction (complete ≠ verbose), Complete Replacement (`STATUS`/`SUPERSEDES`, kein Delta; enge Option-Selection-Ausnahme), vollständige Human-Maintainer-CLI-Befehlsfolgen, Prompt-over-Skill-Ausgabevorrang. Keine SKILL.md-/ADR-Änderung, keine Scripts/Runtime/MCP/Netz, keine Git-/Release-Aktion; ADR-0031/0032 unverändert; v1.1 planning only. Roadmap-Einschub: WP-153/154 neu, bisherige WP-153…157 → **WP-155…159**. **Danach (erst nach Annahme von WP-153): NDF-WP-154 – Skills Pack P0 Hardening.** ADR-0032-Consumer-Scope → separate, eigens autorisierte ADR (Kandidat ADR-0033). WP-130/131/132 bleiben optional/nicht aktiviert.
...
- WP-153: Prompt Execution Contract Baseline — umgesetzt (Annahme erst durch Nova-Review + Human-Maintainer-Commit); `BLOCK_EXECUTION_CONTRACT.md` + Anker in Lifecycle, Prompt Modes, Templates, Git-Standard, Skill Security Policy, Baseline-Guide; Roadmap → WP-151…159; keine SKILL.md-/ADR-Änderung; v1.1 planning only.

## Next Prompt Recommendation

Solange WP-153 nicht committet ist: **Nova-/Human-Maintainer-Review von WP-153** (Review-only-Profil). Nach Annahme (Human-Maintainer-Commit): **NDF-WP-154 – Skills Pack P0 Hardening** als eigener, vollständiger Ausführungsprompt mit Execution Header (`SESSION`/`STATUS`). ADR-0032-Consumer-Scope nur per separater, eigens autorisierter ADR. WP-130/131/132 bleiben optional/nicht aktiviert.
