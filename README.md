...
- **v1.1:** planning started (Validation, Enablement & Operational Maturity; not released, no scope lock) — [v1.1 plan](docs/roadmap/V1_1_PLAN.md) · completed: WP-151, WP-152 ([guide](docs/guides/TOKEN_EFFICIENCY_AND_CONTEXT_BUDGET_BASELINE.md)), WP-153 ([execution contract](framework/prompts/blocks/BLOCK_EXECUTION_CONTRACT.md), committed `70446a3`), WP-154 ([validation](docs/validation/v1-1/SKILLS_PACK_P0_HARDENING.md), committed `07b2884`), and **WP-155 – External Validation Improvement** (G-13 path A: a public, step-evidenced neutral run against a generic, NDF-naive code fixture; PASS WITH NOTES; **G-13 `MATERIALLY_REDUCED` — not closed**, independence unchanged; Human-Maintainer acceptance effective through the commit carrying this state; [validation](docs/validation/v1-1/EXTERNAL_VALIDATION_IMPROVEMENT.md)) · **next planned WP: WP-156 – Project Enablement Validation** (not started; requires its own complete execution prompt)
