# NDF Docs-only Skills MVP

## Purpose

This directory holds the **NDF Docs-only Skills MVP** — a minimal, documentation-only skill pack that helps shorten Claude prompts for repeating NDF work without weakening NDF governance. Each skill encapsulates a stable, public-neutral block of NDF behavior (role, guardrails, closing formats, neutrality, context-pack upkeep) so that a work-package prompt can carry only the WP-specific parts.

Implemented under NDF-WP-129 (Full Prompt Mode), based on the [Skills MVP Implementation Blueprint](../../docs/validation/foundation-0-9/SKILLS_MVP_IMPLEMENTATION_BLUEPRINT.md) (WP-125) and the [Skill Security Policy](../../docs/agent-workflows/NDF_SKILL_SECURITY_POLICY.md) / [ADR-0032](../../docs/adr/ADR-0032-skill-security-policy.md).

## Included Skills (exactly four MVP skills)
...
- No Extended skills (`ndf-release-safety`, `ndf-adr-governance-review`, `ndf-v1-readiness-review`, `ndf-prompt-mode-selector`) — deferred, not implemented here.
